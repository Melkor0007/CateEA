...
from clucls import clustering_dbscan,train_classifier,init_classifier,clustering_pic,clustering,compute_best_mapping,clustering_kmedoids,OpenMaxClassifier,init_openmax_classifier
from newcluster import plot_k_distance,evaluate_stability,plot_clustering_results_2d,plot_confusion_matrix,plot_jaccard_index_heatmap
try:
    from utils import *
    from models import *
    from Load import *
...
class CateEA:
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            train_joint_emb = joint_emb[self.train_ill[:,0]]+joint_emb[self.train_ill[:,1]]
            # print(train_joint_emb.shape)

            num_category = 5
            num_epochs = 100
            clu_start_epoch = 0
            cls_interval = 1
            if epoch ==clu_start_epoch:
                initial_cluster_labels = clustering_kmedoids(train_joint_emb,num_clusters=num_category,verbose=True)
                cluster_labels = initial_cluster_labels
            elif epoch >clu_start_epoch:
                new_cluster_labels = clustering_kmedoids(train_joint_emb,num_clusters=num_category,verbose=True)

                mapping = compute_best_mapping(initial_cluster_labels, new_cluster_labels)
                print(mapping)
                

                remapped_labels = np.array([mapping[label] for label in new_cluster_labels])
                evaluate_stability(cluster_labels,remapped_labels)
                # if epoch %2 ==0 and epoch >=100:
                #     plot_clustering_results_2d(train_joint_emb,cluster_labels,remapped_labels,epoch=epoch)
                    # plot_confusion_matrix(cluster_labels,remapped_labels)
                    # plot_jaccard_index_heatmap(cluster_labels,remapped_labels)
                cluster_labels = remapped_labels

            if epoch >=clu_start_epoch:
                num_classes = len(set(cluster_labels)) - (1 if -1 in cluster_labels else 0)


            # 训练分类器
            if epoch == clu_start_epoch:
                classifier,optimizer,criterion = init_classifier(joint_emb=train_joint_emb,device=device,num_clusters=num_classes)
                train_classifier(train_joint_emb,cluster_labels,classifier,optimizer,device,num_epochs=num_epochs)
            elif epoch >clu_start_epoch and epoch %cls_interval ==0:
                train_classifier(train_joint_emb,cluster_labels,classifier,optimizer,device,num_epochs=num_epochs)

            
            # criterion = nn.CrossEntropyLoss(ignore_index=-1)
            # if epoch >= clu_start_epoch:
            #     classifier, mavs, weibull_params = init_openmax_classifier(train_joint_emb, cluster_labels, num_classes, device, alpha=10)
            ...
            for si in np.arange(0, self.train_ill.shape[0], bsize):
                ...
                if epoch >=clu_start_epoch:
                    end_idx = min(si + bsize, len(cluster_labels))
                else:
                    end_idx = si+bsize
                batch_joint_emb = joint_emb[si:end_idx]

                if epoch >=clu_start_epoch:
                    classifier.eval()
                    # outputs = classifier(batch_joint_emb, mavs, weibull_params)
                    outputs = classifier(batch_joint_emb)
                    batch_cluster_labels = cluster_labels[si:end_idx]

                    batch_cluster_labels_tensor = torch.tensor(batch_cluster_labels, dtype=torch.long).to(device)
                    

                    classification_loss = criterion(outputs, batch_cluster_labels_tensor)

                if epoch %cls_interval ==0 and epoch >=clu_start_epoch:
                    loss_all = loss_joi + in_loss + align_loss + classification_loss
                    cls_loss_all = cls_loss_all + classification_loss.item()
                else:
    ...
    def weighted_test(self, epoch):
        with torch.no_grad():
            ...
            if "100" in self.args.file_dir:
                ...
                acc_r2l = np.zeros((len(top_k)), dtype=np.float32)
                test_total, test_loss, mean_l2r, mean_r2l, mrr_l2r, mrr_r2l = 0, 0., 0., 0., 0., 0.
                left_labels = self.classifier(final_emb[self.test_left]).argmax(dim=1)
                right_labels = self.classifier(final_emb[self.test_right]).argmax(dim=1)


                left_centroids = compute_centroids(final_emb[self.test_left], left_labels)
                right_centroids = compute_centroids(final_emb[self.test_right], right_labels)


                class_distances = pairwise_distances(left_centroids, right_centroids)
                print("-----------distance-------------")
                print(class_distances)
                # class_similarities = torch.exp(-class_distances)

                alpha = 1.0  
                class_similarities = 1 / (1 + alpha * class_distances)

                class_weights = class_similarities / class_similarities.sum(dim=1, keepdim=True)
                print("-----------weight-------------")
                print(class_weights)

                if self.args.dist == 2:
                    distance = pairwise_distances(final_emb[self.test_left], final_emb[self.test_right])
                    weighted_distance = distance * class_weights[left_labels][:, right_labels]
                ...
                if epoch + 1 == self.args.epochs:
                    ...
                    if not os.path.exists(save_path):
                        os.mkdir(save_path)
                    with open(os.path.join(save_path, "pred_weighted.txt"), "w") as f:
                        wr = csv.writer(f, dialect='excel')
                        wr.writerows(to_write)
