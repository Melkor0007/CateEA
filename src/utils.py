...
def compute_centroids(embeddings, labels, num_classes=5):
    """
    Computes centroids for each class based on the embeddings and predicted labels.

    Args:
    - embeddings (torch.Tensor): The embeddings of shape (N, D), where N is the number of entities and D is the dimension of each embedding.
    - labels (torch.Tensor): The predicted class labels of shape (N,), where each value is an integer from 0 to num_classes-1.
    - num_classes (int): The number of distinct classes.

    Returns:
    - centroids (torch.Tensor): A tensor of shape (num_classes, D), representing the centroid of each class.
    """
    # Initialize centroids tensor with zeros
    centroids = torch.zeros((num_classes, embeddings.size(1)), device=embeddings.device)
    
    # Initialize a count tensor to keep track of the number of embeddings in each class
    counts = torch.zeros(num_classes, device=embeddings.device)

    # Accumulate embeddings into their respective class centroids
    for i in range(num_classes):
        mask = (labels == i)  # Boolean mask for the current class
        if mask.sum() > 0:  # Ensure there is at least one sample in this class
            centroids[i] = embeddings[mask].mean(dim=0)
            counts[i] = mask.sum()
    
    # Ensure centroids are properly normalized even if some classes have no embeddings
    # Replace zero centroids with zeros if there are no embeddings for a class
